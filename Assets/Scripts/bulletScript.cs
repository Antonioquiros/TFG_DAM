...
public class bulletScript : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("llave1NV1") || collision.CompareTag("llave2NV2") || collision.CompareTag("llave2NV1") || collision.CompareTag("llave1NV2"))
        {
            return;
        }
        if (collision.CompareTag("enemigo"))
        {
            ControlJugador jugador = FindObjectOfType<ControlJugador>();
            if (jugador != null)
            {
                jugador.RegistrarEnemigoEliminado(); // Llamar al método
            }
